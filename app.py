...
def fig_pair_rolling_corr(price_df: pd.DataFrame, a: str, b: str, window: int):
    if price_df is None or price_df.empty or a not in price_df.columns or b not in price_df.columns:
        fig = go.Figure()
        fig.add_annotation(text="Select valid pair", x=0.5, y=0.5, showarrow=False)
        return fig

    ret = price_df[[a, b]].pct_change().dropna()
